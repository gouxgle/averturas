...
// recepciones con problemas (remitos), oportunidades futuras que llegaron a su fecha
// y entregas programadas (día antes / hora antes), todo no leído, por fecha del evento.
notificaciones.get('/', async (c) => {
  const { rows } = await db.query(`
    ...
    SELECT 'entrega_dia_antes' AS tipo, r.id, r.numero, NULL::timestamptz, NULL::text,
      NULL::numeric, (r.fecha_entrega_est - 1 + TIME '09:00')::timestamptz AS evento_at,
      NULL::text AS recepcion_estado, NULL::text AS recepcion_obs,
      NULL::text AS detalle,
      jsonb_build_object('telefono', cl.telefono, 'direccion_entrega', r.direccion_entrega) AS data,
      json_build_object(
        'nombre', cl.nombre, 'apellido', cl.apellido,
        'razon_social', cl.razon_social, 'tipo_persona', cl.tipo_persona
      ) AS cliente
    ...
    WHERE r.estado = 'emitido'
      AND r.fecha_entrega_est = CURRENT_DATE + 1
      AND r.recordatorio_dia_antes_visto = false

    UNION ALL

    SELECT 'entrega_hora_antes' AS tipo, r.id, r.numero, NULL::timestamptz, NULL::text,
      NULL::numeric,
      ((r.fecha_entrega_est + r.hora_entrega_est)::timestamptz - INTERVAL '1 hour') AS evento_at,
      NULL::text AS recepcion_estado, NULL::text AS recepcion_obs,
      to_char(r.hora_entrega_est, 'HH24:MI') AS detalle,
      jsonb_build_object('telefono', cl.telefono, 'direccion_entrega', r.direccion_entrega) AS data,
      json_build_object(
        'nombre', cl.nombre, 'apellido', cl.apellido,
        'razon_social', cl.razon_social, 'tipo_persona', cl.tipo_persona
    ...
    JOIN clientes cl ON cl.id = r.cliente_id
    WHERE r.estado = 'emitido'
      AND r.hora_entrega_est IS NOT NULL
...
notificaciones.patch('/marcar-leidas', async (c) => {
  ...
  await db.query(
    `UPDATE remitos SET recordatorio_dia_antes_visto = true
     WHERE estado = 'emitido' AND fecha_entrega_est = CURRENT_DATE + 1 AND recordatorio_dia_antes_visto = false`
  );
  await db.query(
    `UPDATE remitos SET recordatorio_hora_antes_visto = true
