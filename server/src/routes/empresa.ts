...
empresa.put('/', async (c) => {
  const { nombre, cuit, telefono, email, direccion, logo_url, objetivo_ventas_mensual, instagram, terminos_url } = await c.req.json();
  ...
  if (existing[0]) {
    const { rows } = await db.query(
      `UPDATE empresa SET nombre=$1, cuit=$2, telefono=$3, email=$4, direccion=$5, logo_url=$6,
         objetivo_ventas_mensual=$7, instagram=$8, terminos_url=$9, updated_at=now()
      ...
      [nombre.trim(), cuit || null, telefono || null, email || null, direccion || null, logo_url || null,
       ...
       terminos_url || null, existing[0].id]
    );
    return c.json(rows[0]);
  } else {
    const { rows } = await db.query(
      `INSERT INTO empresa (nombre, cuit, telefono, email, direccion, logo_url, objetivo_ventas_mensual, instagram, terminos_url)
