...
export function CompactStatsBar({ items }: { items: StatItem[] }) {
  return (
    <div className="relative rounded-xl w-full min-w-0"
      style={{ boxShadow: '0 4px 16px -8px rgba(3,29,73,0.35)' }}>
      ...
      {/* Fade a la derecha — pista visual de que se puede scrollear horizontalmente en mobile */}
      <div className="sm:hidden pointer-events-none absolute top-0 right-0 h-full w-8 rounded-r-xl"
        style={{ background: 'linear-gradient(90deg, transparent, #0a2761)' }} />
    </div>
  );
}
